...
```kotlin
// Save a value
HiPreferencesManager.save(context, "username", "JohnDoe")

// Load a value with a default
val username = HiPreferencesManager.load(context, "username", "DefaultUser")
```
...
if (!context.hasPermissions(blePermissions)) {
    context.requestPermissions(blePermissions, HiGlobal.PermissionReqCodes.BLE)
} else {
...
val resultMap = jsonObject.hiToMap()
...
```kotlin
val sanitizedString = someString.hiSanitize()
```
...
feat_base is available under the MIT License. See the LICENSE file for details.
